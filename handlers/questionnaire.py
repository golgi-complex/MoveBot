...
async def function_client_name(message: types.Message, state: FSMContext):
    if message.text.lower().translate(str.maketrans('', '', string.punctuation)) in CENSURE_SET:
        ...
        await message.answer(data['db_client_name']+', по какому номеру телефона мы можем с Вами связаться?', reply_markup=client_keyboard.contact_kb)
...
def register_handlers_questionnaire(dp: Dispatcher):
    ...
    dp.register_message_handler(function_client_name, state=FSMQuestion.client_name)
    dp.register_message_handler(function_client_phone, state=FSMQuestion.client_phone, content_types=['contact', 'text'])
    dp.register_message_handler(function_check_special, state=FSMQuestion.check_special)
    dp.register_message_handler(function_cargo, state=FSMQuestion.cargo)
    dp.register_message_handler(function_veight, state=FSMQuestion.veight)
    dp.register_message_handler(function_kind_loading, state=FSMQuestion.kind_loading)
    dp.register_message_handler(function_check_pallets, state=FSMQuestion.check_pallets)
    dp.register_message_handler(function_pallets_count, state=FSMQuestion.pallets_count)
    dp.register_message_handler(function_pallets_dimensions, state=FSMQuestion.pallets_dimensions)
    dp.register_message_handler(function_cargo_dimensions, state=FSMQuestion.cargo_dimensions)
    dp.register_message_handler(function_add_carcas, state=FSMQuestion.add_carcas)
    dp.register_message_handler(function_addres_loading, state=FSMQuestion.addres_loading)
    dp.register_message_handler(function_addres_docs, state=FSMQuestion.addres_docs)
    dp.register_message_handler(function_addres_docs_extended, state=FSMQuestion.addres_docs_extended)
    dp.register_message_handler(function_addres_unloading, state=FSMQuestion.addres_unloading)
    dp.register_message_handler(function_date_loading, state=FSMQuestion.date_loading)
    dp.register_message_handler(function_date_unloading, state=FSMQuestion.date_unloading)
    dp.register_message_handler(function_custom, state=FSMQuestion.custom)
    dp.register_message_handler(function_custom_export, state=FSMQuestion.custom_export)
    dp.register_message_handler(function_export_declaration, state=FSMQuestion.export_declaration)
    dp.register_message_handler(function_custom_import, state=FSMQuestion.custom_import)
    dp.register_message_handler(function_insurance, state=FSMQuestion.insurance)
    dp.register_message_handler(function_insurance_cost, state=FSMQuestion.insurance_cost)
    dp.register_message_handler(function_attention, state=FSMQuestion.attention)
    dp.register_message_handler(function_check_data_1, state=FSMQuestion.check_data_1)
    dp.register_message_handler(function_check_data_2, state=FSMQuestion.check_data_2)
    dp.register_message_handler(function_check_data_3, state=FSMQuestion.check_data_3)
